...
| c10/util C++ compile            | `c10/util/.build.ae`              | **Working**        | Compiles 39/42 .cpp files clean (the 3 omissions pull in {fmt}). The inline `_cmake_macros_shim()` |
|                                 |                                   |                    | helper writes a minimal `torch/headeronly/macros/cmake_macros.h` with every `#cmakedefine` resolved |
|                                 |                                   |                    | to /* #undef X */ — same effect as a vanilla CMake configure with no optional features enabled.    |
|                                 |                                   |                    | The `c10/macros/cmake_macros.h` redirect-shim is upstream-shipped and already chained correctly.   |
...
# Compile c10/util (39 of 42 .cpp files; the 3 omissions need {fmt}):
aeb c10/util/.build.ae
```

The cmake_macros.h shim is generated inline by `_cmake_macros_shim()`
in `c10/util/.build.ae`. A future `lib/template` SDK (or a Python-driven
`.codegen.ae`) could canonicalise the "render a CMake `configure_file`
template" shape; today the inline string-builder is the lightest viable
form (one builder, one constant, no template parsing).

The 3 omitted .cpp files (`env.cpp`, `signal_handler.cpp`,
`tempfile.cpp`) all `#include <fmt/format.h>` and need a sibling
`.build.ae` for the {fmt} library. That's the next-natural step.

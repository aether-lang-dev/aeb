...
install: $(INSTALL_TOOLS)
	...
	@SRCH=$$( $(SRCHASH_CMD) ); \
	 ...
	 echo "  (remote build agent NOT installed — opt in with: make install-agent)"

# install-agent — opt-in, deliberate second step. The remote build agent
# is a network-listening server, not core build machinery, so it is NOT
# part of `make install`. This target builds tools/aeb-agent and installs
# the $(BINDIR)/aeb-agent wrapper so a sysop probes the capability by
# `aeb-agent` on PATH. Requires `make install` to have run first (uses the
# installed runtime tree). Built with `ae build --lib lib` (bootstrap-safe;
# does NOT recurse into aeb). The wrapper pins AEB_HOME and puts $(BINDIR)
# on PATH so the agent's own `aeb <target>` runs resolve the installed aeb.
install-agent:
	@test -d "$(SHAREDIR)/tools" || { echo "install-agent: run 'make install' first" >&2; exit 1; }
	$(AETHER) build "$(SHAREDIR)/tools/aeb-agent.ae" -o "$(SHAREDIR)/tools/aeb-agent" --lib "$(SHAREDIR)/lib" $(AEFLAGS)
	@printf '%s\n' \
	    '#!/usr/bin/env bash' \
	    '# aeb-agent wrapper — generated by aeb `make install-agent`' \
	    'export AEB_HOME="$(SHAREDIR)"' \
	    'export PATH="$(BINDIR):$$PATH"' \
	    'exec "$(SHAREDIR)/tools/aeb-agent" "$$@"' \
	    > $(BINDIR)/aeb-agent
	chmod +x $(BINDIR)/aeb-agent
	@echo "installed remote build agent:"
	@echo "  agent:    $(BINDIR)/aeb-agent"

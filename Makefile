...
install: $(INSTALL_TOOLS)
	...
	@printf '%s\n' \
	    ...
	    > $(BINDIR)/aeb
	chmod +x $(BINDIR)/aeb
	@# Standalone aeb-agent wrapper — the remote-aeb build-agent server is
	@# a SEPARATE capability binary (sysop probes `aeb-agent` on PATH), not
	@# an `aeb agent` subcommand. Pins AEB_HOME and puts $(BINDIR) on PATH
	@# so the agent's `aeb <target>` runs resolve the installed aeb.
	@printf '%s\n' \
	    '#!/usr/bin/env bash' \
	    '# aeb-agent wrapper — generated by aeb `make install`' \
	    'export AEB_HOME="$(SHAREDIR)"' \
